...
fn en(key: Key) -> &'static str {
    ...
    match key {
        ...
        ExitHint => "Some countries have few or no exit nodes available. If the connection fails or hangs, try Automatic or a country marked with ★.",
        ExitLegend => "★ = high availability    ⚠ = few exits (may fail)",
        ExitWarningBanner => "The selected exit country has no available exit nodes. Change it in Settings or pick Automatic.",
        ExitDismiss => "Dismiss",
        Automatic => "Automatic",
        ...
        AboutBlurb => "Onionymous is a privacy-first desktop application that routes your \
                       ...
                       Includes pluggable transports (obfs4) for \
...
fn es(key: Key) -> &'static str {
    ...
    match key {
        ...
        ExitHint => "Algunos países tienen pocos o ningún nodo de salida disponible. Si la conexión falla o se cuelga, probá Automático o un país marcado con ★.",
        ExitLegend => "★ = alta disponibilidad    ⚠ = pocos exits (puede fallar)",
        ExitWarningBanner => "El país de salida elegido no tiene nodos disponibles. Cambialo en Settings o elegí Automático.",
        ExitDismiss => "Cerrar",
        ...
        AboutBlurb => "Onionymous es una aplicación de escritorio centrada en la privacidad \
                       ...
                       local. Incluye transportes conectables (obfs4) \
