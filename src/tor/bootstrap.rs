...
impl TorBootstrap {
    ...
    pub async fn disconnect(&self) -> Result<()> {
        ...
        match settings.routing_mode {
            ...
            #[cfg(windows)]
            crate::core::routing::RoutingMode::Tun => {
                if let Err(e) = crate::network::singbox::detener() {
                    log::warn!("Deteniendo sing-box: {}", e);
                }
                let _ = crate::network::proxy::disable_system_proxy();
            }
            #[cfg(not(windows))]
            crate::core::routing::RoutingMode::Tun => {}
        }
            ...
            loop {
                ...
                if state.current_status().is_connected() {
                    ...
                    match settings.routing_mode {
                        ...
                        #[cfg(windows)]
                        crate::core::routing::RoutingMode::Tun => {
                            match crate::network::singbox::iniciar(settings.socks_port) {
                                Ok(_) => {
                                    push_app_log(
                                        &state,
                                        LogLevel::Notice,
                                        format!(
                                            "TUN/VPN activo: todo el tráfico del sistema se enruta vía Tor (sing-box -> SOCKS5 :{})",
                                            settings.socks_port
                                        ),
                                    );
                                }
                                Err(e) => {
                                    push_app_log(
                                        &state,
                                        LogLevel::Error,
                                        format!("Modo TUN falló: {}", e),
                                    );
                                    if let Err(e2) =
                                        crate::network::proxy::enable_system_proxy(settings.socks_port)
                                    {
                                        log::error!("Fallback a proxy falló: {}", e2);
                                    } else {
                                        push_app_log(
                                            &state,
                                            LogLevel::Info,
                                            "Fallback automático a modo Proxy SOCKS5".into(),
                                        );
                                    }
                                }
                            }
                        ...
                        crate::core::routing::RoutingMode::Tun => {
                            push_app_log(
                                &state,
                                LogLevel::Warn,
                                "Modo TUN no soportado en esta plataforma".into(),
                            );
                        }
                    }
                    return;
