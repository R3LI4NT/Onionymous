...
pub fn show(ui: &mut Ui, state: &SharedState, lang: Language) -> HomeActions {
    ...
    draw_exit_warning_banner(ui, state, lang);
...
fn draw_circuit_card(
    ...
    card_with_icon(
        ...
        |ui| {
            ...
            field_label(ui, i18n::t(lang, Key::Mode));
            let mut mode_changed = false;
            egui::ComboBox::from_id_source("mode-combo")
                .width(ui.available_width())
                .selected_text(settings.routing_mode.label())
                .show_ui(ui, |ui| {
                    for &m in RoutingMode::all() {
                        if ui
                            .selectable_value(&mut settings.routing_mode, m, m.label())
                            .changed()
                        {
                            mode_changed = true;
                        }
            ...
            ui.add_space(6.0);
            ui.label(
                RichText::new(i18n::t(lang, Key::ExitLegend))
                    .color(theme::TEXT_DIM)
                    .size(11.0),
            );
            ui.add_space(4.0);
            ui.label(
                RichText::new(i18n::t(lang, Key::ExitHint))
...
fn draw_exit_warning_banner(ui: &mut Ui, state: &SharedState, lang: Language) {
    let warning = state
        .exit_warning
    ...
    egui::Frame::none()
        .fill(Color32::from_rgba_unmultiplied(240, 96, 110, 48))
        .stroke(Stroke::new(
            1.0,
            Color32::from_rgba_unmultiplied(240, 96, 110, 160),
        ))
        .rounding(10.0)
        ...
        .show(ui, |ui| {
            ui.horizontal(|ui| {
                ...
                ui.add_space(8.0);
                ui.label(
                    RichText::new(i18n::t(lang, Key::ExitWarningBanner))
                        .size(13.0)
                        .color(theme::TEXT)
                        .strong(),
                );
                ui.with_layout(Layout::right_to_left(egui::Align::Center), |ui| {
                    let close_btn = egui::Button::new(
                        RichText::new(i18n::t(lang, Key::ExitDismiss))
                            .size(11.5)
                            .color(theme::TEXT),
                    )
