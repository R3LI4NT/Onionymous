...
mod embedded {
    ...
    pub const WINTUN_DLL: &[u8] = include_bytes!(concat!(
        env!("CARGO_MANIFEST_DIR"),
        "/resources/wintun/wintun.dll"
    ));

    pub const SINGBOX_EXE: &[u8] = include_bytes!(concat!(
        env!("CARGO_MANIFEST_DIR"),
        "/resources/singbox/sing-box.exe"
    ));
}
...
pub fn ensure_extracted() -> Result<PathBuf> {
    ...
    #[cfg(feature = "embedded-tor")]
    {
        ...
        extract_if_missing(&dir.join("wintun.dll"), embedded::WINTUN_DLL, false)?;
        extract_if_missing(&dir.join("sing-box.exe"), embedded::SINGBOX_EXE, true)?;
    }
